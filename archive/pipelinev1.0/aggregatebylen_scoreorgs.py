...
for cstrline in ddictContigGroupHits.iterkeys():
	for dstrline in ddictContigGroupHits[cstrline].iterkeys():
		...
		for i in range(len(ddictContigGroupHits[cstrline][dstrline])):
			...
			groupStart.append(float(alignment[7]))
			groupEnd.append(float(alignment[8]))
...
'''
	...
	#Go through the list and determine if anything in the low group can be explained by a different group
	wholecontig = False
	for i in range(len(listOrgHighMid_sort)):
		if wholecontig == False:
			orgA = listOrgHighMid_sort[i][0]
			highSet_orgA = dictOrgSets[orgA]['high']
			midSet_orgA = dictOrgSets[orgA]['middle']
			lowSet_orgA = dictOrgSets[orgA]['low']
			if len(highSet_orgA) == len(ddictContigGroupHits[contig]):
				#print contig, 'Whole contig explained'
				wholeContig = True
				break #If all the groups within a contig are explained by 1 organism at high leve, ignore and skip to the next contig
			#print contig, orgA, 'compared to'
			for j in range(i+1, len(listOrgHighMid_sort)):
				orgB = listOrgHighMid_sort[j][0]
				highSet_orgB = dictOrgSets[orgB]['high']
                        	midSet_orgB = dictOrgSets[orgB]['middle']
                       		lowSet_orgB = dictOrgSets[orgB]['low']	
				#print contig, orgB, 'to match'
				
				if len(lowSet_orgA & highSet_orgB) >= 1 and len(highSet_orgA & lowSet_orgB) >= 1:
					#print contig, orgA, orgB, 'possible high confidence'
					#print dictOrgSets[orgA], dictOrgSets[orgB]
					wholecontig = True
					break
				if len(midSet_orgA & highSet_orgB) >= 1 and len(highSet_orgA & midSet_orgB) >= 1:
					#print contig, orgA, orgB, 'possible mid confidence'
					wholecontig = True
					break
				if len(lowSet_orgA & midSet_orgB) >= 1 and len(midSet_orgA & lowSet_orgB) >= 1: 
					print contig, orgA, orgB, 'possible low confidence'
					wholecontig = True
					break
		else:
			break

			if len(lowSet & highSet) >= 1:
				print contig, len(ddictContigGroupHits[contig]), org6, len(listHighMid[i][2]), nextorg6, len(listHighMid[j][2]), lowSet, highSet, 'high confidence'
				break
			else:
				continue
			break
