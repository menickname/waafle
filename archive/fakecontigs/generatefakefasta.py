...
for astrline in open(args.fakelist):
	...
	contig_status = random.choice(['hgt', 'hgt_cut', 'nohgt'])
	if contig_status == 'hgt' or contig_status == 'nohgt':
		whichend = 'NA'
	else:
	...
	if contig_status == 'hgt' or contig_status == 'hgt_cut':
		section2 = donorGENE.seq
		donorcenter = int(recipient_start) + (len(donorGENE.seq)/2)
	else:
		newrecipientGENE = getRandomGene(recipientGCF)	
		section2 = newrecipientGENE.seq
		donorcenter = int(recipient_start) + (len(recipientGENE.seq)/2)
	newseq = section1 + section2 + section3

	if contig_status == 'hgt' or contig_status == 'nohgt':
		contigstart = donorcenter - contig_len/2
		contigend = donorcenter + contig_len/2
	elif contig_status == 'hgt_cut':
		if whichend == 'start':
			contigstart = donorcenter
			contigend = donorcenter + contig_len
		else:		
			contigstart = donorcenter - contig_len
			contigend = donorcenter
	if contigstart < 0:
		contigstart = 0
	...
	#print recipient_start, donorcenter, recipient_end
