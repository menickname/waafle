...
#We will then see how many organisms with high annotations it takes to fill a contig.
lgtlist = []
...
for contig, orglist, status in lgtlist:
	...
	for taxa in orglist:
		...
		for groups in dictContigOrgLabel[contig][taxa][label]['high']:
			...
			totalgroupcov += combhitlen
			totalgrouplen += grouplen
			totalcomblen += combhitlen
			totalpercID += finalpercID*combhitlen
			totalscore += finalscore*combhitlen
		orginfo = [taxa, totalcoverage, str(totalgroupcov/totalgrouplen), str(totalpercID/totalgrouplen), str(totalscore/totalgrouplen)]
